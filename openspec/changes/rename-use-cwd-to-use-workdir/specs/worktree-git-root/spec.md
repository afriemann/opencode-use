...
working directory (`state.cwd`, set via a prior `use_workdir` call), then the
...
candidates and instructing the caller to call `use_workdir` first, instead of
allowing a raw git subprocess error to propagate.

#### Scenario: Session has an explicit working directory

- GIVEN a session where `use_workdir` was previously called with a path inside a git repository
- WHEN `use_worktree` is invoked
- THEN git operations run in the directory set by `use_workdir`, canonicalized to that repository's top-level directory

#### Scenario: Session was opened outside a git repository with no explicit working directory

- GIVEN a session with no prior `use_workdir` call, where `ctx.directory` is not a git repository and `ctx.worktree` is unset
...
- GIVEN a session with no prior `use_workdir` call, where `ctx.directory` is not a git repository, and the target `path` is not nested under any git repository either
- WHEN `use_worktree` is invoked with `create: true`
- THEN the tool raises a single clear error naming both failed candidates and instructing the caller to call `use_workdir` first, instead of surfacing a raw git subprocess error
